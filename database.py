    ...
    with get_connection() as conn:
        ...
        conn.executemany(
            ...
            ON CONFLICT(kapt_code) DO UPDATE SET
                ...
                household_count = COALESCE(excluded.household_count, apartment_complexes.household_count),
                dong_count = COALESCE(excluded.dong_count, apartment_complexes.dong_count),
                used_date = COALESCE(NULLIF(excluded.used_date, ''), apartment_complexes.used_date),
                address = COALESCE(NULLIF(excluded.address, ''), apartment_complexes.address),
                road_address = COALESCE(NULLIF(excluded.road_address, ''), apartment_complexes.road_address),
                raw_xml = COALESCE(NULLIF(excluded.raw_xml, '{}'), apartment_complexes.raw_xml),
